...
function onDocumentMouseDown(event) {
  // Coordenadas del ratón normalizadas
  const mouse = {
  ...
  const planoZ = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0);
  const punto = new THREE.Vector3();
  raycaster.ray.intersectPlane(planoZ, punto);

  // Calcula la distancia a la estrella
  const dist = punto.distanceTo(estrella.position);

  // Crea un nuevo planeta a esa distancia
  const color = Math.random() * 0xffffff;
  const vel = (Math.random() - 0.5) * 2; // velocidad aleatoria
  Planeta(0.3, dist, vel, color, 1.0, 1.0);
}
